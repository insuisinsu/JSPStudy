...
<%
	...
	*/
	...
	try {
		...
		if(!(rs.next())){	// DB 에 값이 없는 경우
			out.println("DB에 해당 내용이 없습니다.");
		}else{		// DB 에 값이 존재하는 경우, rs 의 값들을 화면에 출력
			//out.println("DB에 값이 존재합니다.");
			String em = rs.getString("email");
			//DB의 email 컬럼의 값이 null 도 아니고 공백도 아니라면 /?? 띄어쓰기는 구분 못함?
...
//			out.println(em);
			...
			// 서블릿으로 출력
			// 서블릿 : JAVA 에서 웹페이지를 출력할 수 있는 JAVA 페이지
